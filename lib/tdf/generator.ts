...
export class TDFGenerator {
  ...
  static updateTDFWithPlayers(
    ...
    try {
      ...
      const metadata: TDFMetadata = {
        ...
        roundtime: parseInt(
          data.querySelector("roundtime")?.textContent || "50"
        ),
        finalsroundtime: parseInt(
          data.querySelector("finalsroundtime")?.textContent || "50"
        ),
        lessswiss: data.querySelector("lessswiss")?.textContent === "true",
        autotablenumber:
          data.querySelector("autotablenumber")?.textContent === "true",
        overflowtablestart: parseInt(
          data.querySelector("overflowtablestart")?.textContent || "16"
        ),
   ...
   */
  private static convertParticipantsToTDFPlayers(
    participants: TournamentParticipant[]
  ): TDFPlayer[] {
    return participants.map((participant) => {
      ...
      // Use the player_id from the participant (this is the TDF player ID)
      const userid = participant.player_id;

      // Format birthdate or use default
      let birthdate = "02/27/2000"; // Default birthdate
      if (participant.player_birthdate) {
        try {
          birthdate = TDFUtils.convertISOToTDFDate(
            participant.player_birthdate
          );
        } catch {
          console.warn(
            `Invalid birthdate for player ${participant.player_name}, using default`
          );
        }
      }
      ...
      try {
        ...
        console.warn(
          `Invalid registration date for player ${participant.player_name}, using current time`
        );
      }
  ...
  private static buildXMLStructure(
    ...
    const playersXML = players
      .map((player) => this.formatPlayerXML(player))
      .join("\n\t\t");
...
<tournament type="${metadata.type}" stage="${metadata.stage}" version="${
      metadata.version
    }" gametype="${metadata.gametype}" mode="${metadata.mode}">
\t<data>
...
\t\t<organizer popid="${metadata.organizer.popid}" name="${this.escapeXML(
...
\t\t</categorycut>`;
   ...
   */
  static validateGeneratedTDF(xmlContent: string): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
